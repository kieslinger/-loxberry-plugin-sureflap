...
if($devices) {	
	...
	foreach($devices AS $i=>$device) {
		...
		if($device['name'] == $_GET['devicename'] OR empty($_GET['devicename'])) {
			...
			// Type			
			print $multi."DeviceTypeID@".$device['product_id']."<br>";
			...
			print $multi."DeviceType@".$devicetype."<br>";
			// Name
			...
			if($device['product_id'] == 1) { 
				...
				print $multi."DeviceLedModeID@".$device_led_id."<br>"; 
				$device_led_id = $device['control']['led_mode'];
				print $multi."DeviceLedMode@".$device_led."<br>"; 	
			}			
			// Flap-data
			if($device['product_id'] == 3 or $device['product_id'] == 6) { 
				...
				print $multi."DeviceLockModeID@".$device_lock_id."<br>";				
				unset($device_lock);
				...
				print $multi."DeviceLockMode@".$device_lock."<br>";
				// Curfew-Status
